...
export default function App() {
  ...
  const latest = (obj, key) => {
    const ts = obj[`Technical Analysis: ${key}`] || obj['Time Series (Daily)'] || obj
  ...
  const analyze = (d) => {
    ...
    const rsi = parseFloat(latest(d.rsi, 'RSI').cur.RSI)
    const macd = latest(d.macd, 'MACD')
    const macdBull = parseFloat(macd.cur['MACD']) > parseFloat(macd.cur['MACD_Signal']) && parseFloat(macd.prev['MACD']) <= parseFloat(macd.prev['MACD_Signal'])
    const adx = parseFloat(latest(d.adx, 'ADX').cur.ADX)
    const bb = latest(d.bbands, 'BBANDS')
    const onLowerBand = parseFloat(dailyTS[Object.keys(dailyTS)[0]]['4. close']) <= parseFloat(bb.cur['Lower Band'])

    const volAvg20 = Object.values(dailyTS).slice(0,20).reduce((a,v) => a + parseFloat(v['6. volume']), 0)/20
    const volToday = parseFloat(Object.values(dailyTS)[0]['6. volume'])
    const volSurge = volToday > volAvg20 * 1.5

    const obvCur = parseFloat(latest(d.obv, 'OBV').cur.OBV)
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1300px', margin: 'auto' }}>
      ...
      {error && <p>Error: {error.message} (check key / rate limit)</p>}
